...
local SET = [[INSERT INTO %s ("sid", "data", "exp") VALUES (%s, %s, TO_TIMESTAMP(%s) AT TIME ZONE 'UTC')]]
local GET = [[SELECT "data" FROM %s WHERE "sid" = %s AND "exp" >= TO_TIMESTAMP(%s) AT TIME ZONE 'UTC']]
local EXPIRE = [[UPDATE %s SET "exp" = TO_TIMESTAMP(%s) AT TIME ZONE 'UTC' WHERE "sid" = %s AND "exp" >= TO_TIMESTAMP(%s) AT TIME ZONE 'UTC']]
local DELETE = [[DELETE FROM %s WHERE "sid" = %s]]


local function exec(self, query, a1, a2, a3)
  ...
  local table_name = pg:escape_identifier(self.table)

  if a1 and a2 and a3 then
    query = fmt(query, table_name, pg:escape_literal(a1), pg:escape_literal(a2), pg:escape_literal(a3))
  elseif a1 and a2 then
    query = fmt(query, table_name, pg:escape_literal(a1), pg:escape_literal(a2))
  else
    query = fmt(query, table_name, pg:escape_literal(a1))
  end
...
function metatable:set(key, value, ttl, current_time)
  return exec(self, SET, key, value, ttl + current_time)
end


function metatable:get(key, current_time)
  local res, err = exec(self, GET, key, current_time)
  if not res then
    return nil, err
  end
  local row = res[1]
...
function metatable:expire(key, ttl, current_time)
  ...
  return exec(self, EXPIRE, exp, key, exp)
end
...
function storage.new(configuration)
  ...
  local tbl               = configuration and configuration.table             --or DEFAULT_TABLE
